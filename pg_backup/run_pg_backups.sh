...
log "   Domyślne: host=${DEFAULT_HOST:-<brak>}, port=${DEFAULT_PORT}, user=${DEFAULT_USER:-<brak>}, retention=$RETENTION_DEFAULT"
...
for i in $(seq 0 $((BACKUP_COUNT - 1))); do
  ...
  log "⏩  [$NAME] host=$HOST port=$PORT db=$DBNAME retention=$RETENTION dir=$BACKUP_DIR opts=${OPTIONS[*]:-—}"
